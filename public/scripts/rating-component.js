'use strict'; // copied from https://overreacted.io/making-setinterval-declarative-with-react-hooks/
...
function FinalRatingComponent() {
  ...
  useInterval(async () => {
    ...
    const rating = await fetch('https://hjsb4f5ur0.execute-api.us-east-1.amazonaws.com/rating').then(res => res.json()).then(data => {
      return data;
    });
    setRating(() => rating);
  }, interval);
